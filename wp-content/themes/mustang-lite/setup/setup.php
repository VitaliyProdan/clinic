			 ...
			 */
				...
				$register_styles = apply_filters( 'wmhook_wm_register_assets_register_styles', array(
					//Frontend
						...
						'stylesheet-global'     => array( '/wp-content/uploads/wmtheme-mustang/global.css' ),
			...
			//Preparing output
				//Archives
					if ( is_day() ) {
						...
						$args['title'] = sprintf( __( '<strong>%s</strong>', 'wm_domain' ), single_cat_title( '', false ) );
					} elseif ( is_tag() ) {
						$args['title'] = sprintf( __( 'Tag Archives: <strong>%s</strong>', 'wm_domain' ), single_tag_title( '', false ) );
					} elseif ( is_search() ) {
						$args['title'] = sprintf( __( 'Результати пошуку: <strong>%s</strong>', 'wm_domain' ), get_search_query() );
					} elseif ( is_author() ) {
