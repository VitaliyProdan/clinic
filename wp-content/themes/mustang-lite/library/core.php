		...
		function wm_logo() {
			//Helper variables
				...
				$args = apply_filters( 'wmhook_wm_logo_args', array(
						...
						'logo_image'  => array( '/wp-content/themes/mustang-lite/assets/img/branding/logo-mustang.png', wm_option( '/wp-content/themes/mustang-lite/assets/img/branding/logo-mustang@2x.png' ) ),
						'logo_type'   => 'text',
		...
		function wm_seo_title( $title, $separator ) {
			...
			//Preparing output
				...
				//general archive
					...
					$title = sprintf( __( '%s', 'wm_domain' ), $title ) . $separator;
		...
		function wm_more( $args = array() ) {
			//Helper variables
				$args = wp_parse_args( $args, apply_filters( 'wmhook_wm_more_defaults', array(
						'attributes' => '',
						'class'      => 'more-link',
						'content'    => sprintf( __( 'Читати <span class="screen-reader-text">the "%s" </span>&raquo;', 'wm_domain' ), get_the_title() ),
	...
	if ( ! function_exists( 'wm_not_found' ) ) {
		function wm_not_found() {
			//Helper variables
				...
				$output .= '<h1>' . __( 'Результатів не знайдено', 'wm_domain' ) . '</h1>';
				$output .= '</article>';
